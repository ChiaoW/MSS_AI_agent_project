...
class CustomMssRM(dspy.Retrieve):
    def __init__(self, db_path: str, k: int = 3):
        super().__init__(k=k)
        # 實例化你原本寫好的 Retriever
        self.internal_retriever = DynamicFewShotRetriever(db_path)
...
# 註冊我們客製化的 Retriever
my_retriever = CustomMssRM(db_path=DB_PATH, k=3)
dspy.settings.configure(lm=llm, rm=my_retriever)

# ==========================================
# 2. 定義 DSPy Signatures (聲明任務的輸入與輸出)
# ==========================================
    ...
    def forward(self, input_text: str, lot_base_name: str = "UnknownLot"):
        ...
        for idx, raw_sample in enumerate(stage1_samples):
            ...
            try:
                ...
                # 🌟 這裡就是你的觀念！直接抓取 DSPy 生成的 reasoning
